...
def process_with_world_and_eq(y, sr, f0, t_world, semitones, eq_rules, residual_gain_db, bandwidth=25):
    ...
    if eq_rules or residual_gain_db != 0:
        y_final = apply_harmonic_eq_stft(y_synthesized, sr, f0_shifted, t_world, eq_rules, residual_gain_db, bandwidth=bandwidth)
    else:
...
def processing_thread(input_paths, output_dir, custom_output_name, suffix, overwrite,
                      ...
                      f0_min, f0_max, is_folder_mode, q, bandwidth):
    try:
        ...
        for i, input_path in enumerate(input_paths):
            ...
            try:
                ...
                q.put(f" > 音声再合成 & EQ処理...")
                y_processed = process_with_world_and_eq(y, sr, f0_std, t_std, pitch_semitones, eq_rules, residual_gain, bandwidth=bandwidth)
...
class VoiceChangerEQApp:
    ...
    def _setup_ui(self):
        ...
        bandwidth_frame = ttk.Frame(eq_frame)
        bandwidth_frame.pack(fill=tk.X, padx=5, pady=5)
        ttk.Label(bandwidth_frame, text="倍音帯域幅 (Hz):").pack(side=tk.LEFT)
        self.bandwidth_var = tk.DoubleVar(value=25.0)  # デフォルト値25 Hz
        ttk.Spinbox(bandwidth_frame, textvariable=self.bandwidth_var, from_=1.0, to=200.0, increment=5.0, width=6).pack(side=tk.LEFT, padx=5)
        ttk.Label(bandwidth_frame, text="(各倍音の周波数範囲。広すぎると重複が発生する可能性あり)").pack(side=tk.LEFT, padx=10)
        # --- 5. 出力 ---
    ...
    def _start_processing(self):
        ...
        threading.Thread(
            ...
            args=(input_paths, out_dir, custom_name, suffix, overwrite, detect_method, model,
                  eq_rules, res_gain, threshold, pitch, f0_min, f0_max, self.is_folder_mode, self.q, bandwidth),
            daemon=True
        ).start()
