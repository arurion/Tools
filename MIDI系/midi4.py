...
class MidiToVideoApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Universal MIDI Renderer v3.0 (Velocity & PitchBend)")
        ...
        self.pitch_range = tk.IntVar(value=12) # ピッチベンド幅(半音)
    ...
    def generate_video_v3(self, midi_path, video_out_path):
        ...
        # bend_data[channel] = ([time1, time2...], [val1, val2...])
        bend_times = {}
        bend_values = {}
        # 初期値(0)を入れておく
        for ch in range(16):
        ...
        for f in range(total_frames):
            ...
            for n in notes:
                ...
                if not is_vertical:
                    # 横モード: 上が高音 (max_note)
                    # key_pos = m_top + (max_note - (note + bend)) * step
                    keys_arr = m_top + (max_note - (n['note'] + b_vals)) * key_step
                else:
                    # 縦モード: 左が低音
                    # key_pos = m_left + (idx + bend) * step
                ...
                if is_vertical:
                    # X = keys_arr, Y = time based
                    if not is_reverse: # Falling (Y decreases as dt increases)
                    ...
                    # Y = keys_arr, X = time based
                    if not is_reverse: # RtoL
            ...
            if is_vertical:
                ...
                if show_piano:
                    ...
                    # 簡易描画: 変化しないのでループ外で描画してもいいが、コード単純化のためここで
