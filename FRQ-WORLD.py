...
import pyworld as pw  # Harvestを含むWORLDライブラリ
...
class Frq:
    ...
    def __init__(self, buf: Optional[bytes] = None, *,
        ...
        self.tool_name: bytes = b'Harvest Generator\x00'[:16]  # ツール名を追加（16バイト以内に収める）
...
def generate_frq_with_harvest(
    data: np.ndarray,
    sample_rate: int,
    per_samples: int = 256
) -> Frq:
    """Harvestアルゴリズムを使い、WAVデータからFRQデータを生成します。"""

    # 1. HarvestでF0推定（pyworld使用）
    f0, timeaxis = pw.harvest(data, sample_rate, frame_period=per_samples / sample_rate * 1000)  # frame_periodをms単位で設定

    # 2. フレーム間隔の調整（Harvestは指定frame_periodで出力するため、補間不要）
    # 無声音部は既に0 Hzとして出力される
...
class FrqGeneratorGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("FRQ Generator with Harvest")
    ...
    def select_directory(self):
        ...
        for i, wav_path in enumerate(wav_files):
            try:
                ...
                frq_instance = generate_frq_with_harvest(wav_data, fs)
