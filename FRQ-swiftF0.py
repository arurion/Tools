...
import librosa
from swift_f0 import SwiftF0  # 正しいインポート
...
class Frq:
    ...
    def __init__(self, buf: Optional[bytes] = None, *,
        ...
        self.tool_name: bytes = b'SwiftF0 Generator\x00'[:16]  # ツール名を追加（16バイト以内に収める）
...
def generate_frq_with_swift_f0(
    data: np.ndarray,
    sample_rate: int,
    per_samples: int = 256
) -> Frq:
    """SwiftF0ライブラリを使い、WAVデータからFRQデータを生成します。"""

    # 1. SwiftF0は16kHzで動作するため、リサンプリングする
    resampled_data = librosa.resample(data.astype(np.float32), orig_sr=sample_rate, target_sr=16000)

    # 2. F0推定（修正: detect_from_arrayメソッドを使用）
    swift_f0_analyzer = SwiftF0(confidence_threshold=0.9)  # 信頼度閾値を設定
    result = swift_f0_analyzer.detect_from_array(resampled_data, 16000)
    
    # PitchResultからf0とvoicingを取得
    f0 = result.pitch_hz
    voicing = result.voicing
    timestamps_resampled = result.timestamps

    # voicingに基づき無声音部を0 Hzに設定
    f0 = np.where(voicing, f0, 0.0)

    # 3. フレーム間隔の調整（補間）
    import scipy.interpolate as interp
    interp_func = interp.interp1d(timestamps_resampled, f0, kind='linear', fill_value=0.0, bounds_error=False)
    ...
    f0_interpolated[f0_interpolated < 0] = 0.0  # 負値を0に設定
...
class FrqGeneratorGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("FRQ Generator with SwiftF0")
    ...
    def select_directory(self):
        ...
        for i, wav_path in enumerate(wav_files):
            try:
                ...
                frq_instance = generate_frq_with_swift_f0(wav_data, fs)
                frq_path = wav_path.replace(".wav", "_wav.frq")
